...
modify_routing_rules() {
    ...
    # 使用jq的--argjson选项直接传递JSON数组
    local rules_json=$(printf '%s\n' "${rules_array[@]}" | jq -s '.')
    
    # 更新路由规则和final
    jq --argjson rules "$rules_json" '.route.rules = $rules | .route.final = "direct-out-main"' "$temp_config" > "$temp_config.new" && mv "$temp_config.new" "$temp_config"
